...
<script>
  ...
  script.src = ${JSON.stringify(widgetUrl)};
...
export default function WidgetPreview({ chatroomId, resumable = false, onSaveBeforeLaunch }: Props) {
  ...
  const [hostnameOverride, setHostnameOverride] = useState(CHATROOM_RUNTIME_API_BASE)
  ...
  const buildHtml = (): string => {
    ...
    return buildPreviewHtml({
      chatroomId,
      resumable,
      apiBaseUrl: IS_LOCAL_DEVELOPMENT
        ? (hostnameOverride || CHATROOM_RUNTIME_API_BASE)
        : CHATROOM_RUNTIME_API_BASE,
      widgetUrl: CHATROOM_WIDGET_URL,
    })
  }
      ...
      {IS_LOCAL_DEVELOPMENT && (
        <div style={{ marginBottom: 12 }}>
          ...
          <Input
            ...
            placeholder={CHATROOM_RUNTIME_API_BASE}
            style={{ maxWidth: 500 }}
          />
        </div>
      )}
