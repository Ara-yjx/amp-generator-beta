...
import { CHATROOM_RUNTIME_API_BASE, CHATROOM_WIDGET_URL } from '../../data/chatroom/config'
...
export function buildEmbedScript(chatroomId: string, resumable = false): string {
  const initBlock = `    StimulizeChatroom.init({
      ...
      apiBaseUrl: "${CHATROOM_RUNTIME_API_BASE}",
...
export default function ScriptGenerator({ chatroomId, resumable = false }: Props) {
  ...
  const generate = () => {
    setSnippet(buildEmbedScript(chatroomId, resumable))
  }
