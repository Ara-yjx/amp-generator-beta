...
function renderItem(props: AmpTrialHtml, rowIndex: number, colIndex: number) {
  const textColor = props.textColor ?? (props.darkMode ? 'white' : 'black');
  return trim(`
    <!-- Item -->
    <div class="spt-trial-content spt-trial-content-${getDisplayKey(rowIndex, colIndex)}" style="margin-left: ${props.concurrentHorizontalGap ?? 0}px; width: ${props.width}px; height: ${props.height}px; position: relative; display: flex; justify-content: center; align-items: center; text-align: center;" data-spt-visible-display="flex">
      <img class="spt-trial-image" style="width: 100%; height: 100% !important; object-fit: contain; display: none;" draggable="false"/>
      <div class="spt-trial-text" style="width: 100%; height: 100%; white-space: ${props.textWrap ? 'pre-line' : 'pre'}; overflow-wrap: break-word; color: ${textColor}; font-weight: ${props.textIsBold ? 'bold' : 'normal'}; font-size: ${props.textFontSize}px; line-height: 1.5em; display: none;"></div>
      <video class="spt-trial-video" style="width: 100%; height: 100%; object-fit: contain; display: none;" preload="auto" autoplay loop playsinline></video>
